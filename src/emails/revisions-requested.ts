...
function messageBody(app: Application, uiLinksInfo: UILinksInfo) {
  ...
  Object.keys(app.revisionRequest).forEach(k => {
    if (!firstRevisionSection && app.revisionRequest[k as keyof Application['revisionRequest']].requested) {
      firstRevisionSection = k;
    }
  });
  const link = linkTemplate.replace(`{id}`, app.appId).replace('{section}', firstRevisionSection);
  return  `
    ${textParagraphSection(`The Data Access Compliance Office (DACO) has reviewed your Application for Access to ICGC Controlled Data.`, { padding: '0px 0px 20px 0px' })}
    ${appInfoBox(app)}
    ${textParagraphSection(`<strong>Your application requires revisions</strong> before granting your project team access to ICGC Controlled Data. Please revise the following:`, { padding: '0px 0px 10px 0px'})}
    ${revisionTable(app)}
  ...
  <mj-section padding="0px 0px 20px 0px">
    <mj-column border="0px #dcdde1 solid" padding="0" >
      <mj-table font-weight="400"
                ...
                ${
                  Object.keys(app.revisionRequest).map((r: string) => {
                    const sectionName = r as keyof Application['revisionRequest'];
                    if (app.revisionRequest[sectionName].requested == false || sectionName == 'general') return;
